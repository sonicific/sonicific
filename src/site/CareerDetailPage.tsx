...
import { jobPostings, openJobPostings } from "../data/jobs";
...
export function CareerDetailPage({ jobId }: CareerDetailPageProps) {
      ...
      <section className="bg-stone-50 py-24">
        <div className="mx-auto max-w-2xl px-4 text-center sm:px-6">
          ...
          <div className="mt-6 flex justify-center">
            <ButtonLink href={routes.careers} icon={ArrowLeft}>
              Xem vị trí khác
            </ButtonLink>
          </div>
        </div>
  ...
  const relatedJobs = openJobPostings
      ...
      <section className="bg-white py-12 sm:py-16">
        <div className="mx-auto grid max-w-6xl gap-10 px-4 sm:px-6 lg:grid-cols-[1fr_20rem] lg:px-8">
          <div className="grid gap-10">
            ...
            />
            <JobSection
              title="Sonic tìm kiếm điều gì"
              items={job.requirements}
              icon={SearchCheck}
            />
            <JobSection
              title="Quyền lợi dành cho bạn"
              items={job.benefits}
              icon={Gift}
            />
          </div>
          ...
          <Reveal variant="right">
            <aside className="sticky top-24 rounded-lg border border-slate-200 bg-stone-50 p-5 shadow-sm">
              ...
              <h2 className="mt-2 font-display text-lg font-semibold text-slate-950">
                {job.title}
              </h2>
              <p className="mt-3 text-sm leading-6 text-slate-600">
                Gửi CV hoặc portfolio, tiêu đề email ghi rõ tên vị trí ứng
                tuyển.
              </p>
              <a
...
function JobSection({ title, items, icon: Icon }: JobSectionProps) {
  return (
    <Reveal>
      <div className="flex items-center gap-3">
        <span className="grid h-10 w-10 place-items-center rounded-lg bg-teal-50 text-teal-700 ring-1 ring-teal-100">
      ...
      <div className="mt-5 grid gap-3">
        {items.map((item) => (
          ...
          >
            <CheckCircle2
              className="mt-1.5 h-4 w-4 shrink-0 text-teal-600"
              aria-hidden="true"
            />
            <span>{item}</span>
          </div>
